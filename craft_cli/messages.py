...
class _PipeReaderThread(threading.Thread):
    ...
    def _write(self, data: bytes) -> None:
        ...
        while True:
            ...
            text = f"{self.prefix}{unicode_line}"
...
class _StreamContextManager:
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        # output command to the pipe-reading thread
        printer.show(stream, text, **printer_flags)

        # enable the thread to read and show what comes through the provided pipe
        self.pipe_reader = _PipeReaderThread(printer, stream, printer_flags, prefix=prefix)
...
class Emitter:
    ...
    @_active_guard()
    def open_stream(self, text: str, prefix: Optional[str] = None):
        """Open a stream context manager to get messages from subprocesses.

        :param text: The preamble to show before the subprocesses start generating messages
        :param prefix: An optional value to prepend to every message generated by
            subprocesses; the default prefix is ":: ". Typically used to indicate the
            step/context that is generating the messages.
        """
        if self._mode == EmitterMode.QUIET:
